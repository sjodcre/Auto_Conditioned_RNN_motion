...
data_kicking_cmu=np.hstack([np.arange(2278, 2279)])
...
network_first = create_regressor(batchsize=batchsize, window=window, input=test.shape[1], dropout=0.0)
...
for i in range(len(X)):
    print('i: ',i)
    #i:0
    print('Y shape: ',test[i:i+1].shape)
    #Y shape:(1,12,240)
    network_func = theano.function([], network(test[i:i+1]))
    #Y_pad_ori=np.array(Y_pad[i:i+1])
    Xorig = np.array(test_gt[i:i+1])
