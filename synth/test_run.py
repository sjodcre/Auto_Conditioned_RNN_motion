...
def motion_chosen(x):
    if x ==0:
        ...
        return np.hstack(np.arange(174,183))
    elif x==3:
        return np.hstack(np.arange(86,95))
    elif x==4:
        return np.hstack(np.arange(97,106))
    elif x==5:
        return np.hstack(np.arange(165,174))
    elif x==6:
...
rng.shuffle(I)
X, Y = X[I], Y[I]
test_fn= np.zeros((1,73,9),dtype='float32')
train_fn=np.zeros((1,3,9),dtype='float32')
for a in range(Y.shape[0]):
	...
	train_fn=np.concatenate((train_fn,train_post.reshape(1,3,9)))
	test_fn=np.concatenate((test_fn,test_post.reshape(1,73,9)))
	print('testing', train_post)
	print('testing reshape',train_post.reshape(1,3,9))

train_fn=train_fn[1:,:]
test_fn=test_fn[1:,:]
print('train final:',train_fn.shape )
#10,3,10
print('test final:',test_fn.shape )
#10,73,10
#train_fn shape = 10 (sample),3(xyz),10(frames)
batchsize = 1
window = test_fn.shape[2]
print('test_fn shape',test_fn.shape[2])
#window = 1
